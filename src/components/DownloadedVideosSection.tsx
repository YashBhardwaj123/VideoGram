...
interface DownloadedVideosSectionProps {
  format: 'mp3' | 'mp4';
  onVideoClick: (videoId: string) => void;
  onDownloadVideo: (video: Video, format: 'mp3' | 'mp4') => Promise<void>;
}

const DownloadedVideosSection: React.FC<DownloadedVideosSectionProps> = ({ 
  format, 
  onVideoClick, 
  onDownloadVideo 
}) => {
      ...
      <div className="mb-6 sm:mb-8">
        <div className="glass-effect rounded-2xl p-4 sm:p-6 lg:p-8 border border-white/20 shadow-2xl">
          <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold bg-gradient-to-r from-white via-blue-200 to-purple-200 bg-clip-text text-transparent">
            Downloaded {format.toUpperCase()} Files
          </h2>
      ...
      {downloads.length === 0 ? (
        <div className="text-center py-16 sm:py-24 lg:py-32">
          <div className="glass-effect rounded-2xl p-6 sm:p-8 border border-white/20 max-w-md mx-auto">
            <div className="text-slate-300 text-lg sm:text-xl font-medium">No downloads yet</div>
            <div className="text-slate-400 mt-2">Start downloading some videos!</div>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
          {downloads.map((video: any, index: number) => (
            <VideoCard 
...
export default DownloadedVideosSection;
